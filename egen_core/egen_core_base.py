...
class EGenCoreBaseModel(GenerationMixin):
    ...
    def __init__(self, model_local_path_or_repo_id, device="cuda:0", dtype=torch.float16, max_seq_len=512,
                 ...
                 hf_token=None, prefetching=True, delete_original=False, trust_remote_code=True):
        """
        ...
        if hf_token is not None:
            self.config = AutoConfig.from_pretrained(self.model_local_path, token=hf_token, trust_remote_code=self.trust_remote_code)
        else:
            self.config = AutoConfig.from_pretrained(self.model_local_path, trust_remote_code=self.trust_remote_code)
    ...
    def get_tokenizer(self, hf_token=None):
        if hf_token is not None:
            return AutoTokenizer.from_pretrained(self.model_local_path, token=hf_token, trust_remote_code=self.trust_remote_code)
        else:
            return AutoTokenizer.from_pretrained(self.model_local_path, trust_remote_code=self.trust_remote_code)
        ...
        if self.get_use_better_transformer():
            try:
                with init_empty_weights():
                    self.model = AutoModelForCausalLM.from_config(self.config, trust_remote_code=self.trust_remote_code)
            ...
            if self.model is None:
                ...
                try:
                    ...
                    with init_empty_weights():
                        self.model = AutoModelForCausalLM.from_config(self.config, attn_implementation="sdpa", trust_remote_code=self.trust_remote_code)
    ...
    def forward(
        ...
        # Make sure memory is clean before standard execution processing
        clean_memory()
